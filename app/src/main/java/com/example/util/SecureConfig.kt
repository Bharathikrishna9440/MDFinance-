...
object SecureConfig {
    ...
    val adminUsername: String by lazy {
        val envVal = try { BuildConfig.ADMIN_USERNAME } catch (e: Throwable) { "" }
        if (envVal.isNotBlank() && envVal != "PLACEHOLDER") envVal else decrypt(CRYPTO_USERNAME)
    }

    val adminPassword: String by lazy {
        val envVal = try { BuildConfig.ADMIN_PASSWORD } catch (e: Throwable) { "" }
        if (envVal.isNotBlank() && envVal != "PLACEHOLDER") envVal else decrypt(CRYPTO_PASSWORD)
    }

    val firebaseDatabaseUrl: String by lazy {
        val envVal = try { BuildConfig.FIREBASE_DATABASE_URL } catch (e: Throwable) { "" }
        if (envVal.isNotBlank() && envVal != "PLACEHOLDER") envVal else "https://collection-app-2007-default-rtdb.asia-southeast1.firebasedatabase.app/"
    }

    val googleScriptUrl: String by lazy {
        val envVal = try { BuildConfig.GOOGLE_SCRIPT_URL } catch (e: Throwable) { "" }
        if (envVal.isNotBlank() && envVal != "PLACEHOLDER") envVal else decrypt(CRYPTO_SCRIPT_URL)
    }
}
