                ...
                for (row in dataRows) {
                    ...
                    if (loanUuidRaw.isNotBlank() || principle > 0.0 || amountDisbursed > 0.0 || interest > 0.0) {
                        ...
                        if (existingLoan == null) {
                            val parsedStartDate = parseDateStringWithFallback(dispersalDateStr) { 0L }
                            existingLoan = allLoansCache.find { 
                                it.customerId == finalCustomerId && 
                                (it.status == "ACTIVE" || (parsedStartDate > 0L && Math.abs(it.startDate - parsedStartDate) < 24 * 60 * 60 * 1000L))
                            }
                        }
